...
import Modelos.LocalizacaoTableModel;
import Relatorios.LocalizacaoPDF;
...
/**
 * Classe que controla as telas e componentes do menu Localização
 * @author Jonatha
 */
public class LocalizacaoTela extends TelaInterna
{
    /**
     * Construtor da classe, que cria o painel e seus componentes.
     * @throws MyException 
     */
    public LocalizacaoTela() throws MyException, ParseException
    {
    ...
    private LocalizacaoTableModel modelo;
    ...
    private void criaListaFiltro()
    {
        ...
        this.painelListaFiltro.add(Formulario.retornaLabel(LocalizacaoLocalizador.getNomeTela(0)+": "));
        this.painelListaFiltro.add(this.filtroCodigo);
        this.painelListaFiltro.add(Formulario.retornaLabel(LocalizacaoLocalizador.getNomeTela(1)+": "));
    ...
    /**
     * Exibe os registros na tabela com base nos filtros de pesquisa e ordenações
     * @throws MyException 
     */
    private void criaListaTabela() throws MyException, ParseException
    {
        this.modelo = new LocalizacaoTableModel();
        
        this.tabela = new TableLista(this.modelo); 
        
        this.tabela.getTableHeader().addMouseListener(new LocalizacaoTela.HeaderListener());
    ...
    private void carregaListaTabela() throws MyException
    {
        ...
        this.modelo.addLocalizacaoLista(LocalizacaoLocalizador.buscaLocalizacoes(this.sqlCondicoes, this.sqlOrdem));
    }
    ...
    private void criaFormularioCampos()
    {
        ...
        this.painelFormularioCampos.add(Formulario.retornaLabel(LocalizacaoLocalizador.getNomeTela(0)+": ", 20));
        this.painelFormularioCampos.add(this.codigo);
        this.painelFormularioCampos.add(Formulario.retornaLabelObrigatoria(LocalizacaoLocalizador.getNomeTela(1)+": ", 60));
    ...
    private class UpdateListener implements ActionListener
    {
        public void actionPerformed(ActionEvent e)
        {
            ...
            if (linhaSelecionada >= 0)
            {
                ...
                try {
                    Localizacao o = LocalizacaoLocalizador.buscaLocalizacao(id);
    ...
    private class DeleteListener implements ActionListener
    {
         public void actionPerformed(ActionEvent e)
         {
            ...
            if (linhaSelecionada >= 0)
            {
                ...
                {
                    try {
                        if(LocalizacaoPersistor.excluiLocalizacao(id))
                            modelo.removeLocalizacao(linhaSelecionada);
                    } catch (MyException ex) {
                        ex.printStackTrace();
                    }
                }
    ...
    private class ReportListener implements ActionListener
    {
        ...
        {
            try {
                LocalizacaoPDF.gerarRelatorioLocalizacao(modelo);
            ...
            else
            {
                try {
                    ...
                    if(codigo.getText().equals(""))
                        ...
                        l.setId(Integer.parseInt(codigo.getText()));

                    l.setNome(nome.getText());
                    
                    if(codigo.getText().equals(""))
                    {                        
                        if( LocalizacaoPersistor.insereLocalizacao(l) )
                            exibeLista();
                    }
                    else
                    {
                        if( LocalizacaoPersistor.alteraLocalizacao(l) )
                            exibeLista();
                    }
                } catch (MyException ex) {
                    ex.printStackTrace();
    ...
    private class HeaderListener implements MouseListener
    {
        ...
        public void mouseClicked(MouseEvent e) {
            try {
                ...
                sqlAnterior = col;
                sqlOrdem = LocalizacaoLocalizador.getConversaoSQL(nomeAtual);           
                carregaListaTabela();
            } catch (MyException ex) {
                ex.printStackTrace();
            }
